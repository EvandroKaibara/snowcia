...
function App() {
  ...
  const [role, setRole] = useState(() => localStorage.getItem('snowcia_role') ?? 'CLIENT')
  ...
  const request = useCallback(async (path, options = {}) => {
    ...
    if (!response.ok) {
      const details = payload.fieldErrors && Object.entries(payload.fieldErrors).map(([field, message]) => `${field}: ${message}`).join(' · ')
  ...
  const logout = useCallback(() => {
    ...
    setToken(null); setRole('CLIENT'); setData(initialData); setEditor(null); setError('')
  }, [])
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [pets, reservations, payments] = await Promise.all([
        request('/api/pets'), request('/api/reservations'), isAdmin ? request('/api/payments') : Promise.resolve([]),
      ])
  ...
  async function handleAuth(event) {
    ...
    try {
      ...
      setRole(nextRole); setToken(payload.token)
  ...
  if (!token) return <AuthScreen {...{ isRegistering, setIsRegistering, form: authForm, setForm: setAuthForm, onSubmit: handleAuth, loading, error }} />
  ...
  return <main className="app-shell">
    <aside className="sidebar"><div className="brand"><span>✦</span> snowcia</div><nav>{navigation.filter(([label]) => label !== 'Pagamentos' || isAdmin).map(([label, icon]) => <button key={label} className={active === label ? 'nav-link active' : 'nav-link'} onClick={() => setActive(label)}><span>{icon}</span>{label}</button>)}</nav><button className="logout" onClick={logout}>↪ Sair</button></aside>
    <section className="content"><header><div><p className="eyebrow">{isAdmin ? 'PAINEL ADMINISTRATIVO' : 'PAINEL'}</p><h1>{active}</h1></div><div className="avatar">🐾</div></header>{error && <p className="form-error page-error">{error}</p>}
      {active === 'Visão geral' && <Dashboard data={data} upcoming={upcoming} paid={paid} isAdmin={isAdmin} onReserve={() => setActive('Reservas')} />}
...
function ReservationRow({ reservation }) { return <div className="reservation-row"><div className="pet-dot">🐾</div><div><strong>{reservation.petName}</strong><small>{formatDate(reservation.checkInDate)} — {formatDate(reservation.checkOutDate)}</small><small className="note">Total: {formatCurrency(reservation.totalAmount)}</small></div><span className="status">{labelOf(reservation.status)}</span></div> }
...
function labelOf(value) { return ({ DOG: 'Cachorro', CAT: 'Gato', BIRD: 'Pássaro', OTHER: 'Outro', PENDING: 'Pendente', CONFIRMED: 'Confirmada', CANCELLED: 'Cancelada', PAID: 'Pago', PIX: 'PIX' })[value] ?? value }
