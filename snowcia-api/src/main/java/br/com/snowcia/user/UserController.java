...
public class UserController {
    ...
    @GetMapping("/clients") public List<UserResponse> clients(@AuthenticationPrincipal AppUser user) { if (!user.isAdmin()) throw new ResponseStatusException(HttpStatus.FORBIDDEN, "Ação restrita à administração"); return users.findAllByRoleNameOrderByNameAsc("CLIENT").stream().map(UserResponse::from).toList(); }
