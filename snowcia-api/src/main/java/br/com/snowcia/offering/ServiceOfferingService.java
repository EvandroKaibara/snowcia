...
public class ServiceOfferingService {
    ...
    public ServiceOfferingResponse toggle(AppUser user, Long id) { requireAdmin(user); var service = find(id); service.update(service.getName(), service.getDescription(), service.getCategory(), service.getTarget(), service.getBillingType(), service.getDurationMinutes(), !service.isActive(), service.isAllowDateSelection(), service.isAllowTimeSelection(), service.isAllowCustomerNotes(), service.isAllowCheckInOut(), service.getMaxPets(), service.getPriceConditions()); return ServiceOfferingResponse.from(repository.save(service)); }
    public void delete(AppUser user, Long id) { requireAdmin(user); repository.delete(find(id)); }
    private ServiceOffering newService(ServiceOfferingRequest r) { return new ServiceOffering(clean(r.name()), clean(r.description()), r.category(), r.target(), r.billingType(), r.durationMinutes(), r.active(), r.allowDateSelection(), r.allowTimeSelection(), r.allowCustomerNotes(), r.allowCheckInOut(), r.maxPets(), prices(r)); }
    private void update(ServiceOffering service, ServiceOfferingRequest r) { service.update(clean(r.name()), clean(r.description()), r.category(), r.target(), r.billingType(), r.durationMinutes(), r.active(), r.allowDateSelection(), r.allowTimeSelection(), r.allowCustomerNotes(), r.allowCheckInOut(), r.maxPets(), prices(r)); }
