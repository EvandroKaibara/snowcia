...
public class ServiceOfferingService {
    ...
    public void delete(AppUser user, Long id, String confirmation) {
        requireAdmin(user);
        if (!"EXCLUIR".equals(confirmation)) throw new org.springframework.web.server.ResponseStatusException(org.springframework.http.HttpStatus.BAD_REQUEST, "Confirmação dupla obrigatória para excluir um serviço");
    ...
    private List<ServiceExtra> extras(ServiceOfferingRequest r) {
        ...
        if (name.contains("day care") || name.contains("daycare")) return List.of(extra("extra_walk", "Passeio extra", 10, "PER_UNIT"), extra("medication_up_to_2", "Medicamento (até 2 medicamentos)", 5, "PER_DAY"), extra("medication_3_plus", "Medicamento (3 ou mais)", 10, "PER_DAY"));
